% attempting to just take the attracting part from elliptic_hyperbolic_lcs

addpath('/Users/jveatch/Documents/MATLAB/LCS-Tool-master'); % path to LCS toolbox

%% load data
load('/Users/jveatch/Documents/MATLAB/Particle_Track_Code/Data/my_data/MARACOOS_filled_zeroes.mat');;
% load('ocean_geostrophic_velocity.mat')
vLat = MARACOOS_filled.V; % here, I made the CODAR data the same size as the demo data to get the grid interpolation to run with current settings, need to fix this later
vLat = permute(vLat, [3 1 2]);
vLon = MARACOOS_filled.U;
vLon = permute(vLon, [3 1 2]);

lat = unique(MARACOOS_filled.lon_gridded);
lon = unique(MARACOOS_filled.lat_gridded);
...
%% Input parameters
T_0 = find(MARACOOS_filled.dnum == datenum('15-Jan_2022 00:00:00'));
% T_0 = 1170;
T_1 = T_0+8;

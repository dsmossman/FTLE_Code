...
% edited just for survey days 29Jan2024
% edited for just tidal components 01Feb2024

load('/home/jmv208/LCS-Tool-master/demo/ocean_dataset/SurveyHours.mat');

load('/home/jmv208/SWARM_data/CODAR_tide_filled.mat');
CODAR_filled = CODAR_tide_filled;
clear CODAR_tide_filled
vLat = CODAR_filled.v_tide; 
vLat = permute(vLat, [3 2 1]);
vLon = CODAR_filled.u_tide;
...
resolutionX = 500; %%CHANGE RESOLUTION HERE
...
k = 1;
%%
for i=1:length(SurveyHours)/2
    date_str = datestr(SurveyHours(i));
    T_0 = find(CODAR_filled.dnum == datenum(SurveyHours(i)));
    T_1 = T_0+12; %% CHANGE INTEGRATION TIME HERE
...
%         close all;
        k = k+1; %counter
        str = strcat('\n hour complete', datestr(SurveyHours(i)));
        fprintf(str);
...
save('ftle_all_surveyDays_FirstHalf_HR_12_TIDES' , 'ftle_all');

...
addpath('Volumes/home/hroarty/codar/MARACOOS/Radials2Totals/totals_toolbox/bin');
...
%% loop through each of the stations to load the last conf.QC.timespan hours of data
...
Rqc(ii).VectorFlag=Rcat.VectorFlag(XFlag(:,end),end);
...
[TUVuwls,RTUV]=makeTotals(Rinterp,'Grid',grid,'TimeStamp',dtime,...
                            'spatthresh',conf.Totals.spatthresh,...
                            'tempthresh',conf.Totals.tempthresh,...
                            'DomaninName',conf.Totals.tempthresh,...
                            'CreationInfo',conf.Totals.CreationInfo);

[TUV,I] = cleanTotals( TUVuwls, conf.Totals.MaxTotSpeed, ...
                            conf.Totals.cleanTotalsVarargin{:} );
fprintf('%d totals removed by cleanTotals\n',sum(I(:)>0))

% Mask totals
%[TUV,I]=maskTotals(TUVclean,conf.Totals.MaskFile,false);
%fprintf('%d totals masked out\n',sum(~I(:)))

% Save results
[tdn,tfn] = datenum_to_directory_filename( conf.Totals.BaseDir, dtime, ...
...
save(fullfile(tdn,tfn{1}),'conf','missingRadials','RTUV','TUVuwls','TUV' )

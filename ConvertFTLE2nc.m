...
ftle_file = uigetfile(".mat","Select the file of FTLE values");
load(ftle_file)

cd 'C:\Users\Delphine\Box\FTLE Work\Processed Data\Gapfilled Data\'
maracoos_file = uigetfile(".mat","Select the file of gapfilled data");
load(maracoos_file)
...
time_readable = char(datetime(ftle_mab.time, 'ConvertFrom','datenum'));
date_readable = unique(string(datetime(ftle_mab.time,'ConvertFrom','datenum','Format','uuuu-MM-dd')));

for i = 1:length(ftle_mab.time)
    cvrg_idx = find(date_readable == string(datetime(ftle_mab.time(i),'ConvertFrom','datenum','Format','uuuu-MM-dd')));

    cvrg = MARACOOS_filled.hfrcvrg(:,:,cvrg_idx);
    cvrg = double(cvrg);
...
% Test plot
% hAxes = setup_figure(flip(ftle_mab.domain));
% plot_ftle(hAxes, flip(ftle_mab.domain), flip(ftle_mab.resolution), ftle_mab.ftle_noEdges(:,:,27));
...
dimid4 = netcdf.defDim(ncid, 'Char', size(time_readable,2));
...
% ftle_mab_2024_winter = ftle_mab_2024_winter(~isnan(ftle_mab_2024_winter));
% ftle_mab_2024_winter = ftle_mab_2024_winter(ftle_mab_2024_winter > 0);
% 
% ftle_mab_2024_spring = ftle_mab_2024_spring(~isnan(ftle_mab_2024_spring));
% ftle_mab_2024_spring = ftle_mab_2024_spring(ftle_mab_2024_spring > 0);
% 
% ftle_mab_2023_spring = ftle_mab_2023_spring(~isnan(ftle_mab_2023_spring));
% ftle_mab_2023_spring = ftle_mab_2023_spring(ftle_mab_2023_spring > 0);
% 
% ftle_mab_2023_fall = ftle_mab_2023_fall(~isnan(ftle_mab_2023_fall));
% ftle_mab_2023_fall = ftle_mab_2023_fall(ftle_mab_2023_fall > 0);
% 
% ftle_full = [ftle_mab_2023_spring;ftle_mab_2023_fall;ftle_mab_2024_winter;ftle_mab_2024_spring];
% 
% prctile(ftle_full,90)
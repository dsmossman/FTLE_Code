...
for year = 2013:2022
    ...
    ftle_file = sprintf('/Volumes/T7_Shield/jmv208/LCS-Tool-master/demo/ocean_dataset/MAB_%d/mab_ftle_fall%d.mat', year, year);
    hfr_file = sprintf('/Volumes/T7_Shield/jmv208/LCS-Tool-master/demo/ocean_dataset/MAB_%d/MARACOOS_filled_%d_fall.mat', year, year);
    ...
    fall_mask = ismember(month(dt), [10, 11, 12]);
    datePartArray = dateshift(dt(fall_mask), 'start', 'day');
    uniqueDates_F = unique(datePartArray);
    numFallDay = numel(uniqueDates_F);

    ftle_mab.time = ftle_mab.time(fall_mask);
    ftle_mab.ftle = ftle_mab.ftle(:, :, fall_mask);
    ...
    outputFile = sprintf('ftle_mab_fall%d.nc', year);

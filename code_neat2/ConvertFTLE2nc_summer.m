...
for year = 2022:2022
    ...
    ftle_file = sprintf('/Volumes/T7_Shield/jmv208/LCS-Tool-master/demo/ocean_dataset/MAB_%d/mab_ftle_summer%d.mat', year, year);
%     ftle_file_sept = sprintf('/Volumes/T7_Shield/jmv208/LCS-Tool-master/demo/ocean_dataset/MAB_%d/mab_ftle_summer%d_Sept.mat', year, year);
    hfr_file = sprintf('/Volumes/T7_Shield/jmv208/LCS-Tool-master/demo/ocean_dataset/MAB_%d/MARACOOS_filled_%d_summer.mat', year, year);
    
%     load(ftle_file_sept);
%     sept = ftle_mab;
    load(ftle_file);
%     ftle_mab.ftle = cat(3, ftle_mab.ftle, sept.ftle);
%     ftle_mab.time = cat(2, ftle_mab.time, sept.time);
    load(hfr_file);
    ...
    summer_mask = ismember(month(dt), [7, 8, 9]);
    datePartArray = dateshift(dt(summer_mask), 'start', 'day');
    uniqueDates_F = unique(datePartArray);
    numsummerDay = numel(uniqueDates_F);

    ftle_mab.time = ftle_mab.time(summer_mask);
    ftle_mab.ftle = ftle_mab.ftle(:, :, summer_mask);
    ...
    outputFile = sprintf('ftle_mab_summer%d.nc', year);
